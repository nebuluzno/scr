defmodule SCR.Agents.ValidatorAgent do
  ...
  def handle_message(%Message{type: :validate, payload: %{content: content, criteria: criteria}, from: from}, state) do
  ...
  def handle_message(%Message{type: :fact_check, payload: %{content: content, claims: claims}, from: from}, state) do
  ...
  def handle_message(%Message{type: :quality_score, payload: %{content: content}, from: from}, state) do
  ...
  defp fallback_validation(content, criteria) do
    ...
    scores = Enum.map(criteria, fn criterion ->
      score = case criterion do
        :accuracy -> 50  # Cannot verify without LLM
        :completeness -> min(100, word_count * 2)  # Rough proxy
  ...
  defp parse_fact_check_response(content) do
    results = Regex.scan(~r/(?:Claim:?\s*)?(.+?)\s*[-:]\s*(VERIFIED|UNVERIFIED|FALSE)/i, content)
  ...
  defp extract_scores(content, criteria) do
    Enum.map(criteria, fn criterion ->
      pattern = "#{criterion}.*?(\\d+)"
      score = case Regex.run(~r/#{pattern}/i, content) do
        [_, score_str] ->
