defmodule SCR.LLM.Metrics do
  ...
  def handle_cast({:track, options}, state) do
    ...
    model_stats =
      Map.get(state.by_model, model_key, %{
        calls: 0,
        prompt_tokens: 0,
        completion_tokens: 0,
        cost: 0.0
  ...
  def handle_call(:stats, _from, state) do
    ...
    stats = %{
      ...
      avg_tokens_per_call:
        if(state.total_calls > 0, do: div(total_tokens, state.total_calls), else: 0),
      avg_cost_per_call:
        if(state.total_calls > 0, do: state.total_cost / state.total_calls, else: 0.0),
