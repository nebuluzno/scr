defmodule SCR.Agents.WriterAgent do
  ...
  def handle_message(%Message{type: :write, payload: %{topic: topic, format: format, style: style}, from: from}, state) do
  ...
  def handle_message(%Message{type: :summarize, payload: %{content: content, max_length: max_length}, from: from}, state) do
  ...
  def handle_message(%Message{type: :transform, payload: %{content: content, target_format: target}, from: from}, state) do
  ...
  defp build_writing_prompt(topic, format, style) do
    format_instruction = case format do
      :markdown -> "Format the output in Markdown with appropriate headers, lists, and emphasis."
      :html -> "Format the output as clean HTML with semantic tags."
      :plain -> "Format as plain text with clear paragraphs."
      :json -> "Format as a JSON object with 'title', 'content', and 'metadata' fields."
      _ -> "Format the output clearly and readably."
    end
